...
try
clearvars -except s f triggerFault1 triggerFault2 triggerParticle1 triggerParticle2
velCats = [2,1.4,0.8];
seed = s;
InitializeSimEnvironment_AllConnected
...
%     fprintf('count: %d; iter: %d; avg compTime: %.2f\n',count,i,avg_timePerVehicle)
    ...
    count = count + 1;
    for k = 1:agents
        connected = all(pdist2(pose(1:2,k)',pose(1:2,k~=1:end)')<50*maxRange);
        if connected
            [~,I] = max(abs(M1-0.5),[],2);
...
%             end
            if all(pdist2(pose(1:2,k)',pose(1:2,k~=1:end)'))<maxRange
                agentFC{k}.otherBots = -1;
            else
                agentFC{k}.otherBots = 0;
            end
    ...
    for k = 1:agents
        %Lidar Scan
        [zCells1,~,~,~] = lidarSim(angles,guessLocs(:,k,k),obsReal,maxRange,res,mapSize);
        [zCells2,~,~,~] = lidarSim(angles,guessLocs(:,k~=1:end,k),obs,maxRange,res,mapSize);
        zCells = [zCells1; zCells2];
        
        [zCellsTrue,rayPts,obsInRange{k},endPts{k}] = lidarSim(angles,pose(:,k),obsReal,maxRange,res,mapSize);
        first_covered
        rayPts_t{i,k} = rayPts;
        %Update known map
        M1(:,k) = updateOccupancyMap(zCellsTrue,M0(:,k),mapSize);
        MGhost(:,k) = updateOccupancyMap(zCells,MGhost(:,k),mapSize);
...
%     makeVideo(F,'SimulationWS/FrontAssignment_1Fault_ExperimentTest')
    ...
    save(sprintf('SimulationWS/SimResultsAllConnected_seed%d_faults%d.mat',seed,f))
    fprintf('Ending...Iters: %d at time %s \n',i,datestr(now,'mm-dd-yy-HHMMSS'))
catch
    yalmip('clear'); clearvars constraints controller u x0 x slack ops
    save(sprintf('SimulationWS/SimResultsAllConnected_seed%d_faults%dNOTCOMPLETE.mat',seed,f))
